...
int main(){
    ...
    for (int i = 0; i < size; i++) {
        ...
        printf("%d %.6f\n", N, ((double)(end - start) / CLOCKS_PER_SEC) * 1000000 / ops); // Time per search operation
    }
    putchar('\n');
    printf("Splay Delete:\n");
    for (int i = 0; i < size; i++) {
        ...
        clock_t start = clock();
        for (int j = 0; j < N; j++) {
            deleteNode(&root, rand() % N);
        }
        clock_t end = clock();

        // printf("n=%d time=%f sec\n", N, (double)(end - start) / CLOCKS_PER_SEC);
        printf("%d %.6f\n", N, ((double)(end - start) / CLOCKS_PER_SEC) * 1e6 / N); // Time per delete operation
    }      
