...
#include <stdlib.h>
#include <time.h>
#include "part3r.h" // или где у тебя код

int main() {
    int n[200] = {10, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 7500, 8000, 8500, 9000, 9500,
    ...
    for(int i = 0; i < size; i++) {
        ...
        HashTable* ht = createTable(N * 2); // load factor ~0.5
    ...
    for(int i = 0; i < size; i++) {
        ...
        int ops = 1000000;
        volatile Node* res = NULL;

        for(int j = 0; j < ops; j++) {
            res = search(ht, rand() % N);
        }

        clock_t end = clock();

        printf("%d %.6f\n", N, ((double)(end - start) / CLOCKS_PER_SEC) * 1000000.0 / ops);
    ...
    for(int i = 0; i < size; i++) {
        ...
        for(int j = 0; j < N; j++) {
            deleteByKey(ht, rand() % N);
        }

        clock_t end = clock();

        printf("%d %.6f\n", N, ((double)(end - start) / CLOCKS_PER_SEC) * 1000000.0 / N);
